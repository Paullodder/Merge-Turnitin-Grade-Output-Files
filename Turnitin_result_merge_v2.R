...
txtfiles <- system("ls -U *.xls", intern=T)

# Read and merge all txt files 
for(file in txtfiles) {
  temp <- read.table(file, header=T, sep="\t")
  
  # Merge tables add current temp table to all users table
  all.users <- merge(all.users, temp, by="username", all.x=T)
